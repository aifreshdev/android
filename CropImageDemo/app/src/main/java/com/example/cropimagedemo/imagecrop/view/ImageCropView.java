...
package com.example.cropimagedemo.imagecrop.view;
...
public class ImageCropView extends ImageView {
    ...
    private boolean mOutTopBottomCropRect;
    private boolean mOutLeftRightCropRect;
    ...
    protected RectF mOriginalRect = new RectF();
    ...
    private boolean savedOutTopBottomCropRect;
    private boolean savedOutLeftRightCropRect;
    ...
    public void resetDisplay() {
        mOutTopBottomCropRect = false;
        mOutLeftRightCropRect = false;
        mBitmapChanged = true;
        resetMatrix();
        requestLayout();
    }
    ...
    protected void getProperBaseMatrix(Drawable drawable, Matrix matrix) {
        ...
        mOriginalRect = getOriginalRectF(matrix);
        if (LOG_ENABLED) {
            printMatrix(matrix);
        }
    }
    ...
    protected RectF getOriginalRectF(Matrix originalMatrix){
        final Drawable drawable = getDrawable();
        RectF bitmapRect = new RectF();
        bitmapRect.set(0, 0, drawable.getIntrinsicWidth(), drawable.getIntrinsicHeight());
        originalMatrix.mapRect(bitmapRect);
        return bitmapRect;
    }
    ...
    protected void panBy(double dx, double dy) {
        ...
        Log.i(LOG_TAG, "panBy: left : " + mScrollRect.left + ", top " + mScrollRect.top);
    ...
    private RectF getAdjust(Matrix supportMatrix) {
        ...
        if(mOriginalRect.left > 0 && mOriginalRect.left > mCropRect.left) {
            if(mOutLeftRightCropRect) {
    ...
    public CropInfo getCropInfo() {
        ...
        if(!mOutLeftRightCropRect){
            left = mCropRect.left - viewImageRect.left;
        }

        if(!mOutTopBottomCropRect){
            top = mCropRect.top - viewImageRect.top;
        }

        return new CropInfo(scale,
                viewBitmap.getWidth(),
                viewImageRect.top + top,
                viewImageRect.left + left,
                mCropRect.top,
                mCropRect.left,
                mCropRect.width(),
                mCropRect.height());
    ...
    public void saveState() {
        ...
        savedOutTopBottomCropRect = mOutTopBottomCropRect;
        savedOutLeftRightCropRect = mOutLeftRightCropRect;
    ...
    public void restoreState() {
        ...
        mOutTopBottomCropRect = savedOutTopBottomCropRect;
        mOutLeftRightCropRect = savedOutLeftRightCropRect;
    ...
    private void checkOutOfCropRect(){
        mOutTopBottomCropRect = false;
        mOutLeftRightCropRect = false;

        RectF rect = getBitmapRect();
        if(rect.top < mCropRect.top
                && rect.bottom > mCropRect.bottom){
            mOutTopBottomCropRect = true;
        }

        if(rect.left < mCropRect.left
                && rect.right > mCropRect.right){
            mOutLeftRightCropRect = true;
        }

        Log.i(LOG_TAG, ">>>>> onScale: " + rect + ", " + mCropRect);
    }
